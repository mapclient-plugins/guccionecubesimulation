...
def usage(prog_name):
    print("Usage: " + prog_name + " <fibre angle>")
...
def simulate(fibre_angle_in, material_parameters, cellml_file, output_dir=None):
    """
    ...
    material_parameters is a list of the four Guccione material parameters.
    """
...
#         exit(-1)
    ...
    # constitutiveParameters = material_parameters
    ...
    dependentField.VariableTypesSet([iron.FieldVariableTypes.U, iron.FieldVariableTypes.DELUDELN, iron.FieldVariableTypes.U1,
    ...
    equationsSetSpecification = [iron.EquationsSetClasses.ELASTICITY, iron.EquationsSetTypes.FINITE_ELASTICITY, constitutiveRelation]
    ...
    guccioneCellMLParameters = [material_parameters[0], 0.0, material_parameters[1], material_parameters[2], material_parameters[3]]
    ...
    CellML.CreateFieldToCellMLMap(dependentField, iron.FieldVariableTypes.U1, 2, iron.FieldParameterSetTypes.VALUES, GuccioneModel,
                                  "equations/E12", iron.FieldParameterSetTypes.VALUES)
    CellML.CreateFieldToCellMLMap(dependentField, iron.FieldVariableTypes.U1, 3, iron.FieldParameterSetTypes.VALUES, GuccioneModel,
                                  "equations/E13", iron.FieldParameterSetTypes.VALUES)
    CellML.CreateFieldToCellMLMap(dependentField, iron.FieldVariableTypes.U1, 4, iron.FieldParameterSetTypes.VALUES, GuccioneModel,
                                  "equations/E22", iron.FieldParameterSetTypes.VALUES)
    CellML.CreateFieldToCellMLMap(dependentField, iron.FieldVariableTypes.U1, 5, iron.FieldParameterSetTypes.VALUES, GuccioneModel,
                                  "equations/E23", iron.FieldParameterSetTypes.VALUES)
    CellML.CreateFieldToCellMLMap(dependentField, iron.FieldVariableTypes.U1, 6, iron.FieldParameterSetTypes.VALUES, GuccioneModel,
                                  "equations/E33", iron.FieldParameterSetTypes.VALUES)
    ...
    for valueIndex, parameter in enumerate(guccioneCellMLParameterIds, 0):
        ...
        print("Setting parameter: " + parameter + "; to value: " + str(guccioneCellMLParameters[valueIndex]) + "; field component: " +
              str(component))
    ...
    def define_problem_solver():
        # Define the problem
        _problem = iron.Problem()
        problemSpecification = [iron.ProblemClasses.ELASTICITY,
                                iron.ProblemTypes.FINITE_ELASTICITY,
                                iron.ProblemSubtypes.FINITE_ELASTICITY_CELLML]
        _problem.CreateStart(problemUserNumber, problemSpecification)
        _problem.CreateFinish()
        
        # Create control loops
        _problem.ControlLoopCreateStart()
        _problem.ControlLoopCreateFinish()
        
        # Create problem solver
        nonLinearSolver = iron.Solver()
        linearSolver = iron.Solver()
        _problem.SolversCreateStart()
        _problem.SolverGet([iron.ControlLoopIdentifiers.NODE], 1, nonLinearSolver)
        ...
        _problem.SolversCreateFinish()
        
        # Create the problem solver CellML equations
        CellMLSolver = iron.Solver()
        _problem.CellMLEquationsCreateStart()
        ...
        _problem.CellMLEquationsCreateFinish()
        
        # Create solver equations and add equations set to solver equations
        solver = iron.Solver()
        _solverEquations = iron.SolverEquations()
        _problem.SolverEquationsCreateStart()
        _problem.SolverGet([iron.ControlLoopIdentifiers.NODE], 1, solver)
        solver.SolverEquationsGet(_solverEquations)
        _solverEquations.sparsityType = iron.SolverEquationsSparsityTypes.SPARSE
        _equationsSetIndex = _solverEquations.EquationsSetAdd(equationsSet)
        _problem.SolverEquationsCreateFinish()
        
        return [_problem, _solverEquations]
    
    def define_boundary_conditions(solver_equations, increment):
        # Prescribe boundary conditions (absolute nodal parameters)
        boundaryConditions = iron.BoundaryConditions()
        solver_equations.BoundaryConditionsCreateStart(boundaryConditions)
    ...
    for counter in range(1, numberOfLoadSteps+1):
        ...
        [problem, solverEquations] = define_problem_solver()
        # define the boundary conditions
        define_boundary_conditions(solverEquations, displacementIncrementDimension)
